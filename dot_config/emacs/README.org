...
At startup, the *scratch* buffer contains a short message, in the form of a Lisp comment, that explains what it is for. This message is controlled by the variable [[elisp:(describe-symbol 'initial-scratch-message)][window-setup-hook]], which should be either a documentation string, or nil (which means to suppress the message).
