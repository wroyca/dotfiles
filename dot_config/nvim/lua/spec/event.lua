local ev = require ("lazy.core.handler.event")
...
local function handle_event (args, event_id, custom_callback)
  vim.schedule (function ()
    if args and args.buf and not vim.api.nvim_buf_is_loaded (args.buf) then
      return
    end
...
--- @param event_id string: Unique identifier for the event (e.g., "LazyFile")
...
local function register_custom_event (event_id, desc, opts)
  if not ev.mappings[event_id] then
    ev.mappings[event_id] = {
      id = event_id,
      event = "User",
      pattern = event_id,
    }
...
register_custom_event (
  "LazyFile",
  "Trigger a deferred user event after a file has been read or created",
  { file_events = { "BufReadPost", "BufNewFile" }, callback = nil }
)
