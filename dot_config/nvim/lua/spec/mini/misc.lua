...
local Spec = {
  ...
  config = function (_, opts)
    ...
    -- caution. That sequence is considered an extension and, in theory, not
    -- all terminal emulators may support it. To avoid relying on it, the
    -- function instead queried the terminal's current background color (via
    -- OSC 11) and cached it with the intention of restoring it later.
    --
    -- In practice, however, this strategy tends to cause more problems than it
    -- solves. Caching a background color at startup assumes that the
    -- terminal's appearance will remain fixed across the editor's lifetime,
    -- but this isn't always the case. For instance, users may switch from a
    -- dark terminal theme to a light one mid-session. In that scenario,
    -- restoring a previously cached dark background into a now-light
    -- environment can produce mismatched or unreadable text (e.g.,
    -- dark-on-dark).
    --
    ...
    MiniMisc.setup_termbg_sync = (function ()
      ...
      -- theme boundaries where the terminal may otherwise retain stale
      -- visuals.
      local sync = function ()
        local normal = vim.api.nvim_get_hl (0, { name = "Normal" })
        if normal.bg then
          -- Emit OSC 11 with the background encoded as a 6-digit RGB hex
          -- string. Most modern terminals interpret this as a request to
          -- change the default background color for the entire display.
      ...
      local reset = function ()
        -- OSC 111 requests the terminal to restore its default background
        -- color to avoid leaving behind any temporary color overrides we
        -- introduced via OSC 11.
      ...
      -- When suspending or exiting, we cleanly return control of the
      -- terminal's background appearance to avoid leaking visual assumptions
      -- into the terminal session the user returns to.
