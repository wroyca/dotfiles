...
-- https://github.com/lucc/nvimpager/blob/56136029d710fb89e118227a2051e68fb0fea39f/lua/nvimpager.lua#L345-L593
...
local function hexformat_rgb_numbers(r, g, b) return string.format("#%06x", r * 2 ^ 16 + g * 2 ^ 8 + b) end
...
local function tokenize(input_string)
  ...
  local function next(input)
    ...
    if next_pos == nil then
      ...
      -- string.
      if next_pos < input:len() then position = next_pos + 1 end
...
function state:state2highlight_group_name()
  ...
  for _, field in pairs(attributes) do
    if self[field] then name = name .. "_" .. field end
  end
  return name
end
...
function state:parse(string)
  for token, c1, c2, c3 in tokenize(string) do
    ...
    if c3 ~= nil then
      self[
        token --[[@as integer]]
      ] = hexformat_rgb_numbers(tonumber(c1), tonumber(c2), tonumber(c3))
    elseif c1 ~= nil then
      ...
      elseif token == 23 or token == 24 or token == 27 or token == 28 or token == 29 then
        ...
        -- supported.
        self[
          attributes --[[@as integer]][token - 20]
        ] = false
...
function state:compute_highlight_command(groupname)
  ...
  if self.foreground ~= "" then
    ...
    if self.ctermfg ~= "" then args = args .. " ctermfg=" .. self.ctermfg end
  end
  if self.background ~= "" then
    args = args .. " guibg=" .. self.background
    if self.ctermbg ~= "" then args = args .. " ctermbg=" .. self.ctermbg end
  end
  local attrs = ""
  for _, key in pairs(attributes) do
    if self[key] then attrs = attrs .. "," .. key end
  end
  attrs = attrs:sub(2)
  if attrs ~= "" then args = args .. " gui=" .. attrs .. " cterm=" .. attrs end
  if args == "" then
...
function state:render(from_line, from_column, to_line, to_column)
  if from_line == to_line and from_column == to_column then return end
