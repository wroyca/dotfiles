...
function _G.format_entry(entry)
  ...
  if entry.valid == 1 then
    ...
    local lnum = entry.lnum > 99999 and -1 or entry.lnum
    local col = entry.col > 999 and -1 or entry.col
    local type = entry.type == "" and "" or " " .. entry.type:sub(1, 1):upper()

    formatted_entry = format_string:format(filename, lnum, col, type, entry.text)
  else
