...
local Spec = {
  ...
  init = function() vim.o.laststatus = 3 end,
    ...
    content = {
      active = function()
        ...
        local modes = setmetatable({
          ...
          __index = function() return { name = "UN", highlight = "%#MiniStatuslineModeOther#" } end,
        })

        MiniStatusline.section_mode = function() return modes[vim.fn.mode()] end
        MiniStatusline.section_filename = function() return vim.bo.buftype == "terminal" and "^t" or "%f%m%r" end
        ...
        MiniStatusline.section_diagnostics = function(args)
          ...
          for severity, info in pairs(severities) do
            ...
            if n > 0 then table.insert(diagnostics_list, ("%%#%s#%s%d"):format(info.hl_group, info.symbol, n)) end
          end
