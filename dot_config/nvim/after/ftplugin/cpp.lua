...
--
-- The issue is that `cinkeys` cannot distinguish between different contexts for
-- `:`. For example, when typing `foo::`, it will reindent after the first colon
-- as if it's a `case` label.
--
-- One possible workaround is to disable `cinkeys` and instead rely on the LSP's
-- *format-on-type* feature. However, that approach applies full formatting, not
-- just indentation, it's often too heavy-handed for real-time use.
--
...
-- difficult to get right. It's a problem in the same spirit as the "Door
-- problem", deceptively simple on the surface, but full of edge cases.
--
